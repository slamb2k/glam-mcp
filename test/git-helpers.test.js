...
export async function testGitHelpers() {
  ...
  try {
    ...
    tests.push({
      ...
      passed: typeof mainBranch === 'string' && ['main', 'master'].includes(mainBranch),
  ...
  try {
    ...
    const branchName2 = generateBranchName('Fix critical bug in payment processing', 'hotfix/');
  ...
  try {
    ...
    tests.push({
      ...
      passed: remoteUrl === null || (typeof remoteUrl === 'string' && remoteUrl.length > 0),
