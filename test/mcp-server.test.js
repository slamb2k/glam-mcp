...
export async function testMCPServer() {
  ...
  try {
    ...
    const hasGitFlowTools = toolNames.some(name => name.startsWith('git_flow_'));
    const hasAutomationTools = toolNames.some(name => ['auto_commit', 'quick_commit', 'smart_commit'].includes(name));
    const hasUtilityTools = toolNames.some(name => ['repo_info', 'analyze_changes', 'list_branches'].includes(name));
    ...
    const autoCommitTool = server.tools.find(tool => tool.name === 'auto_commit');
    if (autoCommitTool) {
      tests.push({
    ...
    const featureStartTool = server.tools.find(tool => tool.name === 'git_flow_feature_start');
    if (featureStartTool) {
      tests.push({
    ...
    const gitFlowTools = server.tools.filter(tool => tool.name.startsWith('git_flow_'));
    const automationTools = server.tools.filter(tool => 
      ['auto_commit', 'quick_commit', 'smart_commit', 'sync_branch', 'squash_commits', 'undo_commit', 'batch_commit'].includes(tool.name)
    );
    const utilityTools = server.tools.filter(tool => 
      ['repo_info', 'analyze_changes', 'list_branches', 'commit_history', 'git_file_status', 
       'show_diff', 'search_code', 'tag_operations', 'stash_operations', 'repo_health_check'].includes(tool.name)
    );
