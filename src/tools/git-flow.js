...
export function registerGitFlowTools(server) {
  ...
  server.addTool({
    ...
    handler: async ({ pr_number, delete_branch = true }) =>
      mergePullRequest(pr_number, delete_branch),
  });
  ...
  try {
    ...
    if (branchExists(featureBranch)) {
      return createErrorResponse(
        `Feature branch '${featureBranch}' already exists`,
      );
    }
...
async function finishFeature(
  ...
  if (!featureBranch.startsWith("feature/")) {
    return createErrorResponse(
      "Not on a feature branch or feature name not provided",
    );
  }
  ...
  try {
    ...
    if (hasUncommittedChanges()) {
      return createErrorResponse(
        "Please commit or stash your changes before finishing the feature",
      );
    }
    ...
    // Create pull request
    const prTitle =
      message || `Feature: ${featureBranch.replace("feature/", "")}`;
...
🤖 Generated with [Slambed MCP](https://github.com/your-username/slambed-mcp)`;
    ...
    if (autoMerge) {
      try {
        execGitCommand(
          `gh pr merge ${prUrl} --squash${deleteBranch ? " --delete-branch" : ""}`,
          { silent: true },
        );
        result.merged = true;
...
async function startRelease(version) {
  ...
  try {
    ...
    if (branchExists(releaseBranch)) {
      return createErrorResponse(
        `Release branch '${releaseBranch}' already exists`,
      );
    }
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      execGitCommand(`git commit -m "chore: bump version to ${version}"`, {
        silent: true,
      });
    }
...
async function finishRelease(version, message) {
  ...
  if (!releaseBranch.startsWith("release/")) {
    return createErrorResponse(
      "Not on a release branch or version not provided",
    );
  }
  ...
  try {
    ...
    if (hasUncommittedChanges()) {
      return createErrorResponse(
        "Please commit or stash your changes before finishing the release",
      );
    }
    ...
    execGitCommand(`git checkout ${mainBranch}`, { silent: true });
    execGitCommand(
      `git merge --no-ff ${releaseBranch} -m "Release ${releaseVersion}"`,
      { silent: true },
    );

    // Create and push tag
    const tagMessage = message || `Release ${releaseVersion}`;
    execGitCommand(`git tag -a v${releaseVersion} -m "${tagMessage}"`, {
      silent: true,
    });
    ...
    try {
      execGitCommand(`git push origin --delete ${releaseBranch}`, {
        silent: true,
      });
    } catch (e) {
...
async function startHotfix(name) {
  ...
  try {
    ...
    if (branchExists(hotfixBranch)) {
      return createErrorResponse(
        `Hotfix branch '${hotfixBranch}' already exists`,
      );
    }
...
async function finishHotfix(name, message, autoMerge, deleteBranch) {
  ...
  if (!hotfixBranch.startsWith("hotfix/")) {
    return createErrorResponse(
      "Not on a hotfix branch or hotfix name not provided",
    );
  }
  ...
  try {
    ...
    if (hasUncommittedChanges()) {
      return createErrorResponse(
        "Please commit or stash your changes before finishing the hotfix",
      );
    }
...
async function createPullRequest(title, description, targetBranch) {
  ...
  try {
    ...
    const prTitle =
      title || `${currentBranch.replace(/^(feature|hotfix|release)\//, "")}`;
    const prBody =
      description ||
      `## ${currentBranch}
