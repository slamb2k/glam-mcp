  ...
  .action(async (action, name, options) => {
    try {
      if (action === 'start') {
        ...
        console.log(chalk.blue(`Finishing feature: ${name || 'current branch'}`));
  ...
  .action(async (action, version, options) => {
    try {
      if (action === 'start') {
          ...
          const answer = await inquirer.prompt([
            {
              ...
              validate: input => /^\d+\.\d+\.\d+$/.test(input) || 'Version must be x.y.z format'
            }
          ]);
        ...
        console.log(chalk.blue(`Finishing release: ${version || 'current branch'}`));
  ...
  .action(async (action, name, options) => {
    try {
      if (action === 'start') {
        ...
        console.log(chalk.blue(`Finishing hotfix: ${name || 'current branch'}`));
